...
//crypt
void crypt(blowfish_vars *vars, unsigned long *left, unsigned long *right) //left,low - 
{   //левый и правый блоки
    int i;

    for (i = 0; i < 16; i++)    //шифрование 16 раундов
    {
        *left ^= vars->P[i]; //XOR-им left с кючами P[i]
        *right ^= F(vars, *left);  //XOR right и F(x)
        swap(right, left);    //Меняем местами
    }

    swap(right, left);
    *right ^= vars->P[16]; //17-й
    *left ^= vars->P[17];    //и 18-й ключ XOR-ся с выходными блоками последнего раунда
}
